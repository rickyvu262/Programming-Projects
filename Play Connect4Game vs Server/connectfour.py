...
def drop_piece(game_state: ConnectFourGameState, column_number: int) -> ConnectFourGameState:
    ...
    empty_row = _find_bottom_empty_row_in_column(game_state.board, column_number)  # return the empty_row or -1 if column is full

    if empty_row == -1: # column is full, raise exception
...
def pop_piece(game_state: ConnectFourGameState, column_number: int) -> ConnectFourGameState:
    ...
    '''
    _require_valid_column_number(column_number) # check valid column (column is not full)
    _require_game_not_over(game_state) # check valid gamestate( no one has won the game yet)
